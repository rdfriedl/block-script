...
export default class AttachTool extends THREE.Group{
	constructor(camera, map, renderer){
		...
		renderer.domElement.addEventListener('mouseup', event => {
			...
			if(
				...
				if(this.onPick && this.pickedBlock)
					this.onPick(this.pickedBlock);
			}
		})
	...
	updateObjects(){
		if(this.enabled && this.target){
			...
			this.selectionFace.position.copy(this.target.target.clone().add(new THREE.Vector3(0.5,0.5,0.5)).multiply(this.map.blockSize));
			this.selectionFace.position.add(this.map.blockSize.clone().divideScalar(2).multiply(this.target.normal).multiplyScalar(1.05));
