...
describe("Room", function() {
	...
	describe("doors", function() {
		it("returns Vector4", function() {
			...
			expect(this.room.doors.equals(new THREE.Vector4(1, 0, 1, 0))).to.equal(
				true,
			);
		});
		...
		it("has rotation applied", function() {
			...
			expect(this.room.doors.equals(new THREE.Vector4(2, 0, 2, 0))).to.equal(
				true,
			);
			this.room.rotation = 1;
			expect(this.room.doors.equals(new THREE.Vector4(1, 0, 2, 0))).to.equal(
				true,
			);
			this.room.rotation = -1;
			expect(this.room.doors.equals(new THREE.Vector4(2, 0, 1, 0))).to.equal(
				true,
			);
		});
	});
