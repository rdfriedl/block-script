...
URL.parseSearch = function(url) {
	...
	if (!URL.parseSearch.cache[url]) {
		var search =
			url.indexOf("?") !== -1
				? url.substr(url.indexOf("?") + 1, url.length + 1)
				: "";
		var queries = search
			.replace(/^\?/, "")
			.replace(/\+/g, " ")
		...
		for (var i = 0; i < queries.length; i++) {
			...
			if (split[0] !== "")
				URL.parseSearch.cache[url][split[0]] = split[1].length
					? window.unescape(split[1])
					: true;
		}
	}
...
Object.clone = function(obj, deep, ignore) {
	...
	// copy properties
	for (var i in obj) {
		if (ignore && (ignore instanceof Array ? ignore.indexOf(i) : i in ignore))
			continue;
...
String.isString = function(v) {
	return v && Object.getPrototypeOf(v).constructor === String;
};
...
Math.lerp = function(min, max, d) {
	...
	if (min < max) return min + dif * d;
	else if (max < min) return max + dif * (1 - d);
	else return min;
};
...
THREE.Texture.prototype.toJSON = function() {
	...
	if (this.magFilter !== THREE.LinearMipMapLinearFilter)
		output.magFilter = this.magFilter;
	if (this.minFilter !== THREE.LinearMipMapLinearFilter)
		output.minFilter = this.minFilter;
	...
	if (this.generateMipmaps !== true)
		output.generateMipmaps = this.generateMipmaps;
