...
describe('MazeGenerator', function() {
	...
	describe('checkPosition', function() {
		it('returns false if position is outside of maze', function() {
			expect(this.generator.checkPosition(new THREE.Vector4(-1,0,0,0))).toBe(false);
			expect(this.generator.checkPosition(new THREE.Vector4(0,-1,0,0))).toBe(false);
			expect(this.generator.checkPosition(new THREE.Vector4(0,0,100,0))).toBe(false);
			expect(this.generator.checkPosition(new THREE.Vector4(0,-2,100,0))).toBe(false);
			expect(this.generator.checkPosition(new THREE.Vector4(5,5,5,0))).toBe(true);
		});
	});

	describe('createCell', function() {
		it('returns cell', function() {
			expect(this.generator.createCell(new THREE.Vector4(5,5,5,5))).toBeDefined();
		});

		it('returns "undefined" if "position" is out of maze', function() {
			expect(this.generator.createCell(new THREE.Vector4(5,-50,5,500))).toBe(undefined);
		});
	...
	describe('getCell', function() {
		...
		it('returns "undefined" if "position" is outside of maze', function() {
			...
			expect(this.generator.getCell(new THREE.Vector4(7,3,50,3))).toBe(undefined);
		});
	});
});
